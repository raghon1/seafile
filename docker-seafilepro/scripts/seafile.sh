...
#Move seahub dir to Volume and make symbolic link
	...
	cp -rp /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media ${STATIC_FILES_DIR}${CCNET_IP}
	#rm -R /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media
	mv /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media.orig
	ln -s ${STATIC_FILES_DIR}${CCNET_IP}/media /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media
#Move avatars to Volume and make symbolic link
	...
	chown -h seafile:seafile /opt/seafile/${SEAFILE_EDITION}-${SEAFILE_VERSION}/seahub/media
